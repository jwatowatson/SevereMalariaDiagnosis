...
model {
   ...
   for(n in 1:N){
      ...
      multi_normal_lpdf(y[n] | mu[1], Sigma[1]),
      log(prev[k])+
      multi_normal_lpdf(y[n] | mu[2], Sigma[2]));
   }
...
generated quantities {
   ...
   for(n in 1:N){
      ...
      real p1;
      real p2;
      p1 = (1-prev[k])*exp(multi_normal_lpdf(y[n] | mu[1], Sigma[1]));
      p2 = prev[k]*exp(multi_normal_lpdf(y[n] | mu[2], Sigma[2]));
