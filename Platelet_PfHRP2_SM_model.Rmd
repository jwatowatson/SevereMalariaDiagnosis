...
X = dat_all[ind_not_na,
            c('log10_hrp2','log10_platelet','log10_parasites')]

mclust_mods_all = Mclust(data = X,G = 3:4)
summary(mclust_mods_all)

mclust_mods_PH = Mclust(data = X[, 1:2],G = 3:4)
summary(mclust_mods_PH)
plot(X[,1], X[,2], col=mclust_mods_PH$classification,
     xlab='HRP2',ylab='Platelet count')

mclust_mods_ParaH = Mclust(data = X[, c(1,3)],G = 3:4)
summary(mclust_mods_ParaH)
plot(X[,1], X[,3], col=mclust_mods_ParaH$classification,
     xlab='HRP2',ylab='Parasite count')

mclust_mods_ParaP = Mclust(data = X[, c(2,3)],G = 3:4)
summary(mclust_mods_ParaP)
plot(X[,2], X[,3], col=mclust_mods_ParaP$classification,
     ylab='Parasite count',xlab='Platelet count')

table(dat_all$study[ind_not_na], mclust_mods_PH$classification)
```
...
stan_dat_all = 
  list(N = nrow(X),
       ...
       prior_mu = matrix(data = c(-log10(c(250, 75)),
                                  log10(c(200, 3000))),
                         nrow = ncol(X),
                         byrow = T),
...
if(RUN_STAN | !file.exists(fname_all_cor)){
  out_all_cor = sampling(lc_mod_cor, data=stan_dat_all, thin=4)
...
hist(dat_all$log10_platelet[ind_rand_notSM])
hist(dat_all$log10_parasites[ind_rand_notSM])

ind_rand_SM = !is.na(dat_all$P_SM1) & (dat_all$P_SM1 > 0.5)
hist(dat_all$log10_platelet[ind_rand_SM])
hist(dat_all$log10_parasites[ind_rand_SM])
```
...
stan_dat_final = 
  list(N = nrow(X),
       ...
       prior_mu = matrix(data = c(-log10(c(300, 250, 75)),
                                  log10(c(1, 200, 3000))),
                         nrow = ncol(X),
                         byrow = T),
