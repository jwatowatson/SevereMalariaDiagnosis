...
model {
   ...
   for(n in 1:N){
      ...
      for(t in 1:Ntest){
         p1 += student_t_lpdf(y[n][t] | 7, mu[t][cluster[t,1]], sigma[t][cluster[t,1]]);
         p2 += student_t_lpdf(y[n][t] | 7, mu[t][cluster[t,2]], sigma[t][cluster[t,2]]);
...
generated quantities {
   ...
   for(n in 1:N){
      ...
      real p2 = 1-prev[k];
      for(t in 1:Ntest){
         p1 = p1*exp(student_t_lpdf(y[n][t] | 7, mu[t][cluster[t,1]], sigma[t][cluster[t,1]]));
         p2 = p2*exp(student_t_lpdf(y[n][t] | 7, mu[t][cluster[t,2]], sigma[t][cluster[t,2]]));
      }
